...
# SEQ_SIZE = 4  # 序列号比特数 L = 4，满足 W + 1 <= 2^L
# WINDOW_SIZE = 8  # 发送窗口大小 W
# TIMEOUT = 3  # 超时时间为 3 秒
# PACKET_LOSS_RATE = 0.1  # 模拟包丢失率
# ACK_LOSS_RATE = 0.1  # 模拟 ACK 丢失率
...
# def send_window_data(sock, addr, data_list, base, next_seq_num, sender_name):
#     for i in range(base, next_seq_num):
#         if i < len(data_list):
#             data = f"{i % (2 ** SEQ_SIZE)}:{data_list[i]}"
#             if not loss_in_loss_ratio(PACKET_LOSS_RATE):
#                 sock.sendto(data.encode(), addr)
#                 print(f"{sender_name}：Sent packet: {data}")
#             else:
#                 print(f"{sender_name}：Packet loss, Seq: {i % (2 ** SEQ_SIZE)}")

# def gbn_sender(sender_ip, sender_port, target_ip, target_port, data_list, sender_name):
#     sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
#         print(f"{sender_name}：Timeout! Resending window...")
#         send_window_data(sock, target_addr, data_list, base, next_seq_num, sender_name)
#         timer.start(timeout_callback)
...
#             if base == next_seq_num:
#                 timer.start(timeout_callback)
#             send_window_data(sock, target_addr, data_list, next_seq_num, next_seq_num + 1, sender_name)
#             next_seq_num += 1
#         lock.release()
...
#             lock.acquire()
#             if ack_num >= base:
#                 print(f"{sender_name}：Received ACK: {ack_num}")
#                 base = ack_num + 1
#                 if base == next_seq_num:
#                     timer.stop()
#                 else:
...
#             # Timeout will trigger the callback
#             continue

#     timer.stop()
#     sock.close()
#     print(f"{sender_name}：All packets sent and acknowledged.")


# def gbn_receiver(receiver_ip, receiver_port, receiver_name):
#     sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
#                 if seq_num == expected_seq_num % (2 ** SEQ_SIZE):
#                     print(f"{receiver_name}：Received packet: {message}")
#                     expected_seq_num += 1
#                 else:
#                     print(f"{receiver_name}：Out of order packet: {message}, expected: {expected_seq_num}")

#                 # 模拟 ACK 丢失
#                 if not loss_in_loss_ratio(ACK_LOSS_RATE):
#                     ack_message = str(expected_seq_num - 1).encode()
#                     sock.sendto(ack_message, addr)
#                     print(f"{receiver_name}：Sent ACK: {expected_seq_num - 1}")
#                 else:
#                     print(f"{receiver_name}：ACK {expected_seq_num - 1} lost")
#             else:
#                 # 处理非数据包，如 quit 信号
#                 if message == 'quit':
#                     print(f"{receiver_name}：Received quit signal.")
#                     break

#         except socket.timeout:
#             continue
#         except KeyboardInterrupt:
#             sock.sendto(b'quit', sender_addr)
#             break

#     sock.close()
#     print(f"{receiver_name}：Receiver stopped.")
